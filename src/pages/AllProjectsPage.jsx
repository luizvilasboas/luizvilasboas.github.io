...
const AllProjectsPage = () => {
  ...
  const filteredProjects = useMemo(() => {
    return allProjects.filter(project => {
      ...
      const matchesSearch = project.name.toLowerCase().includes(searchTerm.toLowerCase()) || project.desc.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div ref={topRef} className="bg-white dark:bg-black py-16 px-8 min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {currentProjects.length > 0 ? (
            ...
            <p className="col-span-full text-center font-mono text-lg dark:text-gray-400">No projects found.</p>
          )}
        </div>
        {totalPages > 1 && (
